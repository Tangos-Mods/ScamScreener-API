...
from app.training_hub.http.live_api_metrics import LiveApiRequestMetrics, merge_live_api_metrics_snapshots
from app.training_hub.http.persistent_api_metrics import (
...
from app.training_hub.core.common import _format_integer_grouped
from app.training_hub.core.content_scrubbing import _scrub_plain_text_value, _scrub_regex_value
...
def test_register_upload_and_dashboard(tmp_path: Path) -> None:
    ...
    assert "queued with 1 cases" in upload.text
    _wait_for_upload_processing(settings)
...
def test_format_integer_grouped_uses_dot_thousands_separator() -> None:
    assert _format_integer_grouped(0) == "0"
    assert _format_integer_grouped(999) == "999"
    assert _format_integer_grouped(1000) == "1.000"
    assert _format_integer_grouped(1000000) == "1.000.000"
    assert _format_integer_grouped(-1234567) == "-1.234.567"
...
def test_linked_client_uploads_appear_in_dashboard_history(tmp_path: Path) -> None:
    ...
    assert anonymous_upload.status_code == 202
    _wait_for_upload_processing(settings)
...
def test_admin_renders_workspace_sidebar_and_primary_controls(tmp_path: Path) -> None:
    ...
    assert response.text.count(">Analytics</span>") == 1
    assert 'href="/admin/analytics/statistics"' in response.text
    assert 'href="/admin/analytics/metrics"' in response.text
...
def test_admin_sidebar_shows_mfa_setup_entry_until_admin_migration_is_complete(tmp_path: Path) -> None:
    ...
    assert 'href="/admin/analytics/statistics"' not in response.text
    assert 'href="/admin/analytics/metrics"' not in response.text
...
def test_admin_subpages_render_separate_operational_areas(tmp_path: Path) -> None:
    ...
    scrub_page = client.get("/admin/scrub-rules")
    runs_page = client.get("/admin/runs")
    system_page = client.get("/admin/system")
    statistics_page = client.get("/admin/analytics/statistics")
    metrics_page = client.get("/admin/analytics/metrics")
    ...
    assert scrub_page.status_code == 200
    assert "Content Scrubbing Rules" in scrub_page.text
    assert "No scrub rules configured." in scrub_page.text
    assert scrub_page.text.count('<details class="workspace-disclosure" open>') == 1
    assert re.search(r'href="/admin/scrub-rules"[^>]*aria-current="page"', scrub_page.text) is not None
    ...
    assert statistics_page.status_code == 200
    assert "Statistics" in statistics_page.text
    assert "Future trend dashboards" in statistics_page.text
    assert statistics_page.text.count('<details class="workspace-disclosure" open>') == 1
    assert re.search(r'href="/admin/analytics/statistics"[^>]*aria-current="page"', statistics_page.text) is not None
    assert re.search(r'<summary class="workspace-disclosure-toggle is-open">\s*<span>Analytics</span>', statistics_page.text) is not None

    assert metrics_page.status_code == 200
    assert "Current process and host load" in metrics_page.text
    assert "/api/v1/health" in metrics_page.text
    assert "/api/v1/metrics" in metrics_page.text
    assert metrics_page.text.count('<details class="workspace-disclosure" open>') == 1
    assert re.search(r'href="/admin/analytics/metrics"[^>]*aria-current="page"', metrics_page.text) is not None
    assert re.search(r'<summary class="workspace-disclosure-toggle is-open">\s*<span>Analytics</span>', metrics_page.text) is not None


def test_admin_can_create_and_delete_content_scrub_rules(tmp_path: Path) -> None:
    settings = _settings(tmp_path)
    client = TestClient(create_app(settings))
    ...
    created = _post_form(
        client,
        "/admin/scrub-rules",
        data={"pattern_text": "discord.gg/", "match_mode": "contains"},
        follow_redirects=True,
    )
    assert created.status_code == 200
    assert "Created content scrub rule #1" in created.text
    assert "discord.gg/" in created.text

    with training_db.connect(settings.database_path) as connection:
        rule_row = connection.execute(
            "SELECT match_mode, pattern_text, use_regex, match_count, is_enabled FROM content_scrub_rules WHERE id = 1"
        ).fetchone()
        assert rule_row == ("contains", "discord.gg/", 0, 0, 1)

    deleted = _post_form(client, "/admin/scrub-rules/1/delete", follow_redirects=True)
    assert deleted.status_code == 200
    assert "Deleted content scrub rule #1" in deleted.text
    assert "No scrub rules configured." in deleted.text

    with training_db.connect(settings.database_path) as connection:
        remaining = connection.execute("SELECT COUNT(*) FROM content_scrub_rules").fetchone()[0]
        assert int(remaining) == 0


def test_content_scrub_rules_apply_to_future_uploads(tmp_path: Path) -> None:
    settings = _settings(tmp_path)
    client = TestClient(create_app(settings))
    ...
    _post_form(
        client,
        "/admin/scrub-rules",
        data={"pattern_text": "discord\\.gg/[A-Za-z0-9]+", "match_mode": "contains", "use_regex": "1"},
        follow_redirects=True,
    )
    ...
    upload = _post_form(
        ...
        files={
            "training_file": (
                ...
                _valid_payload(messages='[{"sender":"system","text":"Visit discord.gg/abc123 for prizes"}]'),
                "application/x-ndjson",
            )
        },
    )
    assert upload.status_code == 202
    assert "Content scrubbing removed 1 matches across 1 fields." in upload.text

    with training_db.connect(settings.database_path) as connection:
        connection.row_factory = training_db.Row
        case_row = connection.execute(
            "SELECT case_id, payload_json, source_upload_id FROM training_cases WHERE case_id = ?",
            ("case_000001",),
        ).fetchone()
        upload_row = connection.execute("SELECT stored_path FROM uploads WHERE id = 1").fetchone()
        rule_row = connection.execute("SELECT match_count FROM content_scrub_rules WHERE id = 1").fetchone()

    assert case_row is not None
    assert upload_row is not None
    assert rule_row is not None
    assert str(case_row["case_id"]) == "case_000001"
    assert "discord.gg/" not in str(case_row["payload_json"])
    assert int(rule_row["match_count"]) == 1

    stored_payload = Path(str(upload_row["stored_path"])).read_text(encoding="utf-8")
    assert "discord.gg/" not in stored_payload
    assert '"caseId":"case_000001"' in stored_payload

    scrub_page = client.get("/admin/scrub-rules")
    assert scrub_page.status_code == 200
    assert "Hits" in scrub_page.text
    assert (
        re.search(
            r"<td>Contains</td>\s*<td>Yes</td>\s*<td><code>discord\\\\\.gg/\[A-Za-z0-9\]\+</code></td>\s*<td>1</td>",
            scrub_page.text,
        )
        is not None
    )
...
def test_content_scrub_match_modes_support_plain_text_and_regex() -> None:
    assert _scrub_plain_text_value("secret", "secret", "exact") == ("", 1)
    assert _scrub_plain_text_value("secret value", "secret ", "starts_with") == ("value", 1)
    assert _scrub_plain_text_value("value-suffix", "-suffix", "ends_with") == ("value", 1)
    assert _scrub_plain_text_value("x bad y bad", "bad", "contains") == ("x  y ", 2)

    assert _scrub_regex_value("secret-42", re.compile(r"secret-\d+"), "exact") == ("", 1)
    assert _scrub_regex_value("secret-42 value", re.compile(r"secret-\d+"), "starts_with") == (" value", 1)
    assert _scrub_regex_value("value secret-42", re.compile(r"secret-\d+"), "ends_with") == ("value ", 1)
    assert _scrub_regex_value("a 123 b 456", re.compile(r"\d+"), "contains") == ("a  b ", 2)


def test_live_api_request_metrics_tracks_active_and_recent_requests() -> None:
    metrics = LiveApiRequestMetrics()

    public_token = metrics.start_request("/api/v2/lowestbin", "Mozilla/5.0")
    internal_token = metrics.start_request("/api/v1/health", "ScamScreener/2.0.0+1.21.1")
    snapshot = metrics.snapshot()

    assert public_token is not None
    assert internal_token is not None
    assert snapshot["totalActive"] == 2
    assert snapshot["totalToday"] == 2
    assert snapshot["totalLast60s"] == 2
    assert snapshot["totalSinceStart"] == 2
    assert snapshot["publicApi"]["active"] == 1
    assert snapshot["publicApi"]["requestsToday"] == 1
    assert snapshot["internalApi"]["active"] == 1
    assert snapshot["internalApi"]["requestsToday"] == 1
    assert {entry["agent"] for entry in snapshot["entries"]} == {"Browser (HTTPS)", "2.0.0 1.21.1"}

    metrics.finish_request(public_token)
    metrics.finish_request(internal_token)
    after_finish = metrics.snapshot()

    assert after_finish["totalActive"] == 0
    assert after_finish["totalToday"] == 2
    assert after_finish["publicApi"]["requestsLast60s"] == 1
    assert after_finish["internalApi"]["requestsLast60s"] == 1
    assert len(after_finish["entries"]) == 2


def test_admin_metrics_page_exposes_live_api_window(tmp_path: Path) -> None:
    client = TestClient(create_app(_settings(tmp_path)))
    ...
    client.get("/api/v1/health", headers={"User-Agent": "ScamScreener/2.0.0+1.21.1"})
    client.get("/api/v2/lowestbin", headers={"User-Agent": "Mozilla/5.0"})

    metrics_page = client.get("/admin/analytics/metrics")

    assert metrics_page.status_code == 200
    assert "Push-updated request window" in metrics_page.text
    assert "This day" in metrics_page.text
    assert "All Time" in metrics_page.text
    assert "Endpoints and agents" in metrics_page.text
    assert 'data-stream-url="/admin/analytics/metrics/stream"' in metrics_page.text
    assert '"internalApi"' in metrics_page.text
    assert '"requestsLast60s": 1' in metrics_page.text or '"requestsLast60s":1' in metrics_page.text
    assert "/api/v2/lowestbin" in metrics_page.text
    assert "/api/v1/health" in metrics_page.text
    assert "Browser (HTTPS)" in metrics_page.text
    assert "2.0.0 1.21.1" in metrics_page.text


def test_merge_live_api_metrics_snapshots_combines_totals_and_endpoint_rows() -> None:
    merged = merge_live_api_metrics_snapshots(
        {
            "totalActive": 0,
            "totalToday": 4,
            "totalLast10s": 3,
        ...
        },
        {
            ...
            "entries": [
                ...
                {
                    "endpoint": "/api/v1/bazaar",
                    "agent": "ScamScreener MarketGuard",
                    "requestsLast10s": 1,
                    "requestsLast60s": 1,
                    "requestsToday": 1,
                    "totalSinceStart": 7,
    ...
    assert merged["totalToday"] == 7
    assert merged["totalLast10s"] == 5
    assert merged["totalLast60s"] == 7
    assert merged["totalSinceStart"] == 22
    assert merged["publicApi"]["requestsToday"] == 7
    assert merged["publicApi"]["totalSinceStart"] == 22
    assert merged["publicApi"]["requestsPerSecond10s"] == 0.5
...
def test_persistent_api_metrics_survive_app_restart(tmp_path: Path) -> None:
    settings = _settings(tmp_path)
    ...
    with TestClient(create_app(settings)) as restarted_client:
        _post_form(
            restarted_client,
            "/login",
            data={"username_or_email": "dev", "password": "supersecret"},
            follow_redirects=True,
        )
        metrics_page = restarted_client.get("/admin/analytics/metrics")

    assert metrics_page.status_code == 200
    assert '"totalSinceStart": 2' in metrics_page.text or '"totalSinceStart":2' in metrics_page.text
    assert "/api/v2/lowestbin" in metrics_page.text
    assert "/api/v1/health" in metrics_page.text


def test_persistent_api_metrics_recorder_accumulates_today_and_all_time(tmp_path: Path) -> None:
    recorder = PersistentApiMetricsRecorder(str(tmp_path / "metrics.db"))
    recorder.record_request("public_api", "/api/v2/lowestbin", "Browser (HTTPS)")
    recorder.record_request("public_api", "/api/v2/lowestbin", "Browser (HTTPS)")
    recorder.record_request("internal_api", "/api/v1/health", "ScamScreener 1.21.1")

    assert recorder.flush() is True

    reloaded = PersistentApiMetricsRecorder(str(tmp_path / "metrics.db"))
    snapshot = reloaded.snapshot(focus_entries=[("/api/v2/lowestbin", "Browser (HTTPS)")])

    assert snapshot["totalToday"] == 3
    assert snapshot["totalSinceStart"] == 3
    assert snapshot["publicApi"]["requestsToday"] == 2
    assert snapshot["publicApi"]["totalSinceStart"] == 2
    assert snapshot["internalApi"]["requestsToday"] == 1
    assert snapshot["entries"][0]["endpoint"] == "/api/v2/lowestbin"
    assert snapshot["entries"][0]["requestsToday"] == 2
    assert snapshot["entries"][0]["totalSinceStart"] == 2


def test_combine_live_and_persistent_api_metrics_prefers_persistent_today_and_all_time() -> None:
    combined = combine_live_and_persistent_api_metrics(
        {
            "totalActive": 1,
            "totalToday": 1,
            "totalLast10s": 2,
    ...
    assert combined["totalActive"] == 1
    assert combined["totalLast10s"] == 2
    assert combined["totalToday"] == 9
    assert combined["totalSinceStart"] == 99
    assert combined["entries"][0]["requestsLast10s"] == 2
    assert combined["entries"][0]["requestsToday"] == 9
    assert combined["entries"][0]["totalSinceStart"] == 99
...
def test_admin_overview_shows_case_upload_statistics(tmp_path: Path) -> None:
    settings = _settings(tmp_path)
    client = TestClient(create_app(settings))
    ...
    _post_form(
        client,
        "/dashboard/upload",
        files={"training_file": ("training-cases-v2.jsonl", _valid_payload(case_id="case_stats_today"), "application/x-ndjson")},
    )
    _post_form(
        client,
        "/dashboard/upload",
        files={"training_file": ("training-cases-v2.jsonl", _valid_payload(case_id="case_stats_3_days"), "application/x-ndjson")},
    )
    _post_form(
        client,
        "/dashboard/upload",
        files={"training_file": ("training-cases-v2.jsonl", _valid_payload(case_id="case_stats_9_days"), "application/x-ndjson")},
    )

    three_days_ago = (datetime.now(timezone.utc) - timedelta(days=3)).isoformat().replace("+00:00", "Z")
    nine_days_ago = (datetime.now(timezone.utc) - timedelta(days=9)).isoformat().replace("+00:00", "Z")
    with training_db.connect(settings.database_path) as connection:
        connection.execute(
            "UPDATE training_cases SET created_at = ?, updated_at = ? WHERE case_id = ?",
            (three_days_ago, three_days_ago, "case_stats_3_days"),
        )
        connection.execute(
            "UPDATE training_cases SET created_at = ?, updated_at = ? WHERE case_id = ?",
            (nine_days_ago, nine_days_ago, "case_stats_9_days"),
        )
        connection.commit()
    ...
    assert admin_page.status_code == 200
    assert "Statistics" in admin_page.text
    assert "Last 7d: 2" in admin_page.text
    assert "Avg: 0.30/day" in admin_page.text
...
def test_https_enforcement_falls_back_to_https_public_base_url_for_trusted_proxy(tmp_path: Path) -> None:
    client = TestClient(
        create_app(
            _settings(
                tmp_path,
    ...
    response = client.get(
        "/",
        headers={"Host": "scamscreener.creepans.net", "X-Forwarded-Proto": "http"},
        follow_redirects=False,
    )
...
def _post_form(
    ...
    response = client.post(path, data=form_data, files=files, headers=request_headers, follow_redirects=follow_redirects)
    if path == "/dashboard/upload":
        _wait_for_upload_processing(client.app.state.settings)
    return response


def _wait_for_upload_processing(settings: TrainingHubSettings, timeout_seconds: float = 2.0) -> None:
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        with training_db.connect(settings.database_path) as connection:
            pending = int(
                connection.execute(
                    "SELECT COUNT(*) FROM uploads WHERE status IN ('queued', 'processing')"
                ).fetchone()[0]
            )
        if pending == 0:
            return
        time.sleep(0.02)
    raise AssertionError("Timed out waiting for background upload processing.")
